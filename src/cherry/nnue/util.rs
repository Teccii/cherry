...
pub fn feed_forward(
    ...
    for i in 0..(HL / NativeVec::CHUNKS_16) {
        ...
        unsafe {
            ...
            let stm_weight = NativeVec::load(weights.out_weights.as_ptr().add(bucket_offset + offset));
            let ntm_weight = NativeVec::load(weights.out_weights.as_ptr().add(bucket_offset + HL + offset));
