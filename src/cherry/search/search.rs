    ...
    if !Node::PV && !in_check && skip_move.is_none() {
        ...
        */
        ...
        if depth < W::rfp_depth()
            ...
            && !beta.is_loss()
        {
            return Score(estimated_score.0 + W::rfp_lerp() * (beta.0 - estimated_score.0) / 1024);
        }
        ...
        */
        ...
        we can prune this node.
        */
        let nmp_margin = W::nmp_margin(improving, depth).max(0) as i32;
        ...
        if !Node::ROOT && !best_score.is_loss() {
            if is_noisy {
                ...
                */
                ...
                if !in_check && lmr_depth <= W::fp_depth() && estimated_score + fp_margin <= alpha {
                    move_picker.skip_quiets();
                }
