...
weights! {
    ...
    rfp_corr_scale | RFP_CORR_SCALE: i64 => 512  | 256..=768;
...
impl W {
    ...
    #[inline]
    pub const fn rfp_margin(depth: i32, corr: i32, improving: bool) -> i64 {
        let depth = depth as i64;
        let abs_corr = corr.abs() as i64;
        ...
        let corr_scale = W::rfp_corr_scale() * abs_corr / 1024;

        base + scale1 + scale2 + corr_scale
    }

    #[inline]
    pub const fn razor_margin(depth: i32, improving: bool) -> i64 {
        let depth = depth as i64;
        let depth_scale = DEPTH_SCALE as i64;
        let (base, scale1, scale2) = if improving {
            (
    ...
    #[inline]
    pub const fn fp_margin(depth: i32, hist_score: i32, improving: bool) -> i64 {
        let depth = depth as i64;
        let depth_scale = DEPTH_SCALE as i64;
        let (base, scale1, scale2) = if improving {
