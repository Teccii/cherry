...
fn rev_futile<Node: NodeType>(
    ...
    !Node::PV
        && depth < 12
        && !alpha.is_decisive()
        && eval > beta + rev_futile_margin(entry, depth, improving)
}

#[inline(always)]
fn rev_futile_margin(entry: Option<TTData>, depth: u8, improving: bool) -> i16 {
...
fn nmp<Node: NodeType>(
    ...
    Node::NMP
        && depth > 4
        && ctx.search_stack[ply as usize - 1].move_played.is_some()
        && eval >= beta
        && !board.colors(board.stm()).is_subset(board.pieces(Piece::Pawn) | board.pieces(Piece::King))
}

#[inline(always)]
fn nmp_depth(depth: u8) -> u8 {
    depth.saturating_sub(3 + depth / 3)
}
...
#[inline(always)]
fn quiet_pruning<Node: NodeType>(board: &Board, ply: u16, is_capture: bool, best_score: Option<Score>) -> bool {
    !Node::PV
        && ply != 0
        && !is_capture
        && best_score.map_or(false, |s| !s.is_decisive())
        && !board.colors(board.stm()).is_subset(board.pieces(Piece::King) | board.pieces(Piece::Pawn))
}
...
fn futile(
    ...
    !board.in_check()
        && lmr_depth < 12
        && eval <= alpha - futile_margin(lmr_depth, improving)
}
...
#[inline(always)]
fn double_ext_margin<Node: NodeType>() -> i16 {
    -4 + 244 * Node::PV as i16
}

#[inline(always)]
fn triple_ext_margin<Node: NodeType>() -> i16 {
...
pub fn search<Node: NodeType>(
    ...
    if !in_check && skip_move.is_none() && !alpha.is_decisive() && !beta.is_decisive(){
        ...
        */
        if rev_futile::<Node>(tt_entry, depth, static_eval, alpha, beta, improving) {
    ...
    while let Some(mv) = move_picker.next(pos, &ctx.history) {
        ...
        let hist = ctx.history.get_move(pos.board(), mv);
        ...
        */
        if let Some(entry) = tt_entry && singular(entry, mv, depth, ply) {
            ...
            if s_score < s_beta {
                ...
                if s_score < s_beta - double_ext_margin::<Node>() {
                    extension += 1;
                }

                if s_score < s_beta - triple_ext_margin::<Node>() {
                    extension += 1;
                }
        ...
        */
        ...
        reduction += 926 * !Node::PV as i32;
        reduction += 926 * improving as i32;

        if quiet_pruning::<Node>(pos.board(), ply, is_capture, best_score) {
            //Late Move Pruning
