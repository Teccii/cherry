...
impl Move {
    ...
    fn convert_uci(&mut self, board: &Board) {
        let backrank = Rank::First.relative_to(board.stm());
        let uci_castle_src = Square::new(File::E, backrank);
        let uci_castle_short = Square::new(File::G, backrank);
        let uci_castle_long = Square::new(File::C, backrank);

        let rights = board.castle_rights(board.stm());
        let (from, to) = (self.from(), self.to());

        if board.king(board.stm()) == from && from == uci_castle_src {
            if to == uci_castle_short && let Some(rook) = rights.short {
                self.set_dest(Square::new(rook, backrank));
            }

            if to == uci_castle_long && let Some(rook) = rights.long {
                self.set_dest(Square::new(rook, backrank));
            }
        }
    }
...
impl FromStr for Move {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        fn parse(s: &str) -> Option<Move> {
            ...
            let to = s.get(0..2)?.parse::<Square>().ok()?;
            let promotion = s.as_bytes().get(4).map(|&s| {
                let piece = Piece::try_from(char::from(s)).ok();

                piece.filter(|&p| matches!(p, Piece::Pawn | Piece::King))
            }).flatten();

            Some(promotion.map_or_else(
                || Move::new(from, to, MoveFlag::None),
                |p| Move::new_promotion(from, to, p)
            ))
        }
